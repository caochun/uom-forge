...
第二阶段 A 不提供 JSON Schema；B 的业务输入只有 A 的结果，另外提供 `output-contract.ts` 中的紧凑格式说明。完整 JSON Schema 留在本地校验，不再重复展开到提示词。B 不接收第一阶段业务说明、原文、用户对话或完整旧模型。本轮先识别概念、关系和业务行为，不细化属性和输入字段；本地也检查属性和输入为空、业务过程支撑尚未评估。每次 API 调用只发送当前阶段输入，B 不继承 A 的额外上下文。
