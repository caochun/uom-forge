...
try {
  ...
  const basisThought = simple.page.locator('.business-basis .reading-reasoning')
  const basisReasoning = '先区分事实与未决问题。\n再组织业务故事。'
  await emit(simple.page, { type: 'delta', part: 'basis', text: '先区分事实与未决问题。\n', reasoning: true })
  await emit(simple.page, { type: 'delta', part: 'basis', text: '再组织业务故事。', reasoning: true })
  await expect(basisThought).toHaveAttribute('open', '')
  await expect(basisThought.getByRole('region', { name: '模型思考过程' })).toHaveText(basisReasoning)
  await expect(basisThought.locator('summary')).toContainText('实时输出中')
  await simple.page.screenshot({ path: path.join(artifacts, 'basis-thinking.png'), fullPage: true })
  await simple.page.setViewportSize({ width: 390, height: 844 })
  await simple.page.getByRole('button', { name: '收起建模助手', exact: true }).click()
  await simple.page.screenshot({ path: path.join(artifacts, 'basis-thinking-mobile.png'), fullPage: true })
  assert.ok(await simple.page.evaluate(() => document.documentElement.scrollWidth <= innerWidth), 'basis reasoning must wrap on mobile')
  await simple.page.setViewportSize({ width: 1440, height: 1000 })
  await simple.page.getByRole('button', { name: '建模助手', exact: true }).click()
  await emit(simple.page, { type: 'delta', part: 'basis', text: '事实：客户提交订单。' })
  await expect(basisThought).not.toHaveAttribute('open', '')
  await expect(simple.page.locator('.business-basis > .narrative-body')).toHaveText('事实：客户提交订单。')
  await basisThought.locator('summary').click()
  await expect(basisThought.getByRole('region', { name: '模型思考过程' })).toHaveText(basisReasoning)
  ...
  await emit(simple.page, { type: 'delta', part: 'semantic', reasoning: true, text: '先判断对象边界。\n再检查关系方向。' })
  const designThought = simple.page.locator('.reading-narrative > .reading-reasoning')
  await expect(designThought).toHaveAttribute('open', '')
  await expect(designThought.getByRole('region', { name: '模型思考过程' })).toHaveText('先判断对象边界。\n再检查关系方向。')
  await expect(designThought.locator('summary')).toContainText('实时输出中')
  await emit(simple.page, { type: 'delta', part: 'semantic', text: '订单是需要独立保存的业务对象。' })
  await expect(designThought).not.toHaveAttribute('open', '')
  ...
  await emit(simple.page, { type: 'delta', part: 'design-check', reasoning: true, text: '先检查订单归属。' })
  const checkThought = simple.page.locator('.design-review .reading-reasoning')
  await expect(checkThought.getByRole('region', { name: '模型思考过程' })).toHaveText('先检查订单归属。')
  ...
  await expect(simple.page.locator('.design-review')).toContainText('先检查订单归属。')
  ...
  await emit(simple.page, { type: 'delta', part: 'semantic', reasoning: true, text: '重新检查客户与订单的归属。' })
  ...
  await designThought.locator('summary').click()
  await expect(designThought.getByRole('region', { name: '模型思考过程' })).toHaveText('重新检查客户与订单的归属。')
  ...
  await expect(basisThought.locator('summary')).toContainText('已结束')
  await simple.page.waitForTimeout(50)
  await basisThought.evaluate(element => {
    const details = element as HTMLDetailsElement
    if (!details.open) (details.querySelector('summary') as HTMLElement).click()
  })
  await expect(basisThought.getByRole('region', { name: '模型思考过程' })).toHaveText(basisReasoning)
  await expect(simple.page.locator('.business-basis > .narrative-body')).not.toContainText('先区分事实')
  await expect(simple.page.locator('.todo-bar')).toHaveCount(0)
  await simple.page.setViewportSize({ width: 390, height: 844 })
  ...
  const interruptedBasis = await open(simpleDraft)
  await interruptedBasis.page.getByRole('button', { name: '重新建模', exact: true }).click()
  await emit(interruptedBasis.page, { type: 'phase', part: 'basis', text: '正在整理业务依据。' })
  await emit(interruptedBasis.page, { type: 'delta', part: 'basis', text: basisReasoning, reasoning: true })
  await interruptedBasis.page.getByRole('button', { name: '停止', exact: true }).click()
  await expect(interruptedBasis.page.locator('.business-basis')).toContainText('本次尚未生成业务依据正文，已保留思考内容。')
  await expect(interruptedBasis.page.locator('.reading-reasoning > summary')).toContainText('本次输出已保留')
  await interruptedBasis.page.getByRole('button', { name: '保存草稿', exact: true }).click()
  await interruptedBasis.page.reload()
  await nav(interruptedBasis.page).getByRole('button', { name: '03 建模' }).click()
  await interruptedBasis.page.locator('.reading-reasoning > summary').click()
  await expect(interruptedBasis.page.getByRole('region', { name: '模型思考过程' })).toHaveText(basisReasoning)
  await interruptedBasis.page.locator('.workspace-heading .secondary-button').filter({ hasText: '重新建模' }).click()
  await expect(interruptedBasis.page.locator('.business-basis .reading-reasoning')).toHaveCount(0)
  await emit(interruptedBasis.page, { type: 'phase', part: 'basis', text: '正在整理业务依据。' })
  await emit(interruptedBasis.page, { type: 'delta', part: 'basis', text: '新一轮思考。', reasoning: true })
  await expect(interruptedBasis.page.getByRole('region', { name: '模型思考过程' })).toHaveText('新一轮思考。')
  await interruptedBasis.page.getByRole('button', { name: '停止', exact: true }).click()
  await interruptedBasis.context.close()
