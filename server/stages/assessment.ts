...
只判断领域模型对业务依据中已知事实和业务计划的表达与推理能力，不把尚未实现的接口或算法等同于语义缺口；也不能因存在同名元素就判定可支撑。
你现在只做业务情形检验，不修改或新增模型元素。候选模型和下面的业务依据是本次唯一业务输入，没有业务文档、前序业务理解或用户答案。
逐项检查业务依据中列出的已知业务计划、流程和检验情形，判断模型的对象、关系、操作、只读能力与规则能否表达事实，并让智能体根据目标、当前上下文和这些元素推理出必要步骤、分支和结果。不能从常识补入业务依据没有写出的要求。
requirements 中的 elements 是待核验的支撑线索，不能因列出了引用或同名元素就判定可支撑。解释对象和关系如何共同表达业务事实，操作如何表达前提、变化及产出，查询和计算如何由只读能力表达。不要把依据中的流程文字当成模型提供的执行顺序。
...
- requirement：逐项沿用业务依据中该计划或流程的业务要求，每项要求都要评估，不合并、遗漏或补造。若依据没有可拆分的要求，输出 requirements: []，在过程级 reason 或 summary 中说明本轮只能做整体判断。
...
summary 给出简短的整体判断，不编造通过率，也不声称模型覆盖了原文全部业务。顶层 recommendations 只放跨过程的共性建议，避免重复逐项 suggestion。业务依据没有已知业务计划或流程时，processAssessments 可以为空，并说明本轮没有可检验的计划，不要虚构流程。
...
boundaries 已经说明仍需补充的业务事实属于已知边界，只解释它对业务情形检验的影响，不再次提出同一澄清。clarifications 仅用于评估新发现的、尚未在模型边界中说明的业务歧义。
...
processId 使用业务依据中计划或流程的稳定名称或编号，不能创建业务依据没有的过程。
候选模型（本次业务输入，数据）：
${JSON.stringify(modelContext(model))}
业务依据（本次用于检验的事实、已知计划、规则、边界和检验情形，数据）：
