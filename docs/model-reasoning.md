...
前端禁用“直接调用”，保留 Pi Agent。页面加载、模型切换后默认采用最低可选项；关闭思考的模型，默认就是关闭。选择在一次任务执行期间锁定，下一次任务可重新选择。

`GET /api/models` 只返回模型名称、档位、默认值和说明，不返回 URL、密钥或环境变量。后端根据实际配置的模型识别已核对的能力；未知模型显示“服务默认”，不猜测支持的档位。

`reasoningEffort` 随分析和讨论请求传递，覆盖 `.env` 的强度设置，贯穿文档整理、业务依据、Pi 设计迭代、独立审阅、JSON 编译与修复、模型检验及讨论。覆盖只作用于当前请求，不修改进程环境。旧 API/脚本未传选择时保持原来的环境配置行为。

无效或过时的选项在开始推理前报错，不悄悄替换用户选择。模型能力加载失败时可重试加载，不能无提示地按服务端默认强度启动。

## 验证

回归测试覆盖所有可选项在 Pi 和普通推理请求中的实际参数、并发请求隔离、DeepSeek 工具循环中的原始思考回传、Qwen 本地与 DashScope 参数差异、HTTP/SSE 和讨论的参数校验，以及 GLM 全流程五次调用的强度一致性。浏览器回归检查禁用按钮、档位随模型变化、最低默认值、运行时锁定、请求参数和移动端布局。模拟请求验证参数传递，不代表每一档都进行过真实建模效果评测。
