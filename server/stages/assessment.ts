...
你现在只做业务情形检验，不修改或新增模型元素。候选模型和下面的建模依据是本次唯一业务输入，没有业务文档、前序业务理解或用户答案。
逐项检查建模依据中列出的已知业务计划或流程，判断模型的对象、关系、操作、只读能力与规则能否表达事实，并让智能体根据目标、当前上下文和这些元素推理出必要步骤、分支和结果。建模依据中的检验情形是这些计划的具体核验材料，应在对应解释中沿用；不能从常识补入建模依据没有写出的要求。
...
summary 给出简短的整体判断，不编造通过率，也不声称模型覆盖了原文全部业务。顶层 recommendations 只放跨过程的共性建议，避免重复逐项 suggestion。建模依据没有已知业务计划或流程时，processAssessments 可以为空，并说明本轮没有可检验的计划，不要虚构流程。只有独立检验情形而没有计划或流程时，也不要创建虚构的 processId；这些情形已由设计阶段的表达检查处理。
...
建模依据（本次用于检验的事实、已知计划、规则、边界和检验情形，数据）：
