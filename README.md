...
工作台有四个页面：业务文档、业务理解、建模、模型检验。模型检验内包含模型自述和业务过程支撑，下一轮建模反馈也在此页填写。建模页把过程、制品和状态分开：运行面板在执行时展开六步进度和当前活动，空闲时收起为上轮结果摘要（元素数、表达检查、事实覆盖、耗时），展开可查看运行记录和调用耗时；页面顶部一条聚合待办列出未保存答案、待确认问题、过期结果和整理警告，取代分散提示；建模工作区分三个标签——业务依据（事实、故事及每项事实的覆盖状态与映射元素合并展示）、建模决策、模型视图（图、集合、元素详情和业务表达检查），标签徽标携带计数与警示。运行期间不再自动切换标签。前端使用 React TSX，后端、共享契约、验证脚本和 Vite 入口统一使用 TypeScript。
...
各次调用均可停止；B 失败时保留说明、语义草稿和旧模型，发送 `{ stage: 'compile', semanticPlan, narrative, semantic, provider }` 单独重试，无须重跑事实、故事或 A。B 的模型调用接收建模说明及已校验语义计划，narrative 只用于请求边界校验和后续业务表达检查。只有严格结构校验通过才更新图，不静默删除错误引用。建模页用“业务依据”展示事实、故事、原句和覆盖映射，用“业务表达检查”展示检查与修正结果；本轮通过不代表已证明全部业务覆盖。检查超时或停止时，已完成候选仍可查看。页面不提供原始输出记录区域，业务理解、建模说明和模型自述继续在各自正文中流式显示；原始输出完整保留在本地草稿中用于诊断。运行进度、耗时及停止按钮在主区域可见，建模助手可收起并保留各阶段对话。
