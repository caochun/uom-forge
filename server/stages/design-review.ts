...
${CHECK_METHOD}
本轮检查：
- 沿用业务依据已经给出的检验情形，只用当前设计尝试表达。
- 依据没有单列情形时，从事实或故事中选择少量具体情形；不能因为没有标题或编号就要求重新生成依据。
- 可以根据同一份依据补充遗漏的关键情形，但不能删除或替换已有情形来规避缺口。
- 说明参与者怎样绑定事项，操作怎样改变状态，只读能力怎样提供结果，规则怎样约束前提、效果和例外。
